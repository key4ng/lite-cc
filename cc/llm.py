...
_oci_signer_lock = threading.Lock()
...
def _make_fc_id() -> str:
    """Generate a Responses API compatible ID starting with 'fc_'."""
    return "fc_" + uuid.uuid4().hex[:24]
...
def _convert_messages_to_responses_input(messages: list[dict]) -> tuple[str, list[dict]]:
    ...
    system_parts = []
    input_items = []
    # Map original call_* IDs to fc_* IDs so tool results can reference them
    ...
    for msg in messages:
        ...
        if role == "system":
            ...
            original_id = msg["tool_call_id"]
            fc_id = call_id_map.get(original_id, original_id)
            input_items.append({
